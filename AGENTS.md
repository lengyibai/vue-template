...
> 编辑代码时，如发现当前文件未按本规范执行，应及时按照文档规范进行优化并补齐（例如缺少注释则补齐），确保与本规范一致，注意不要出现中文乱码。
...
- 读取代码文件时，统一直接按 UTF-8 读取，不使用依赖系统默认编码的读取方式。
- 写入代码文件时，统一直接按 UTF-8 处理；如需使用脚本或命令直接写回文件，必须显式指定 UTF-8 编码。
...
- 不要在页面组件目录、页面文件夹或组件文件夹内部创建 `.test.js`、`.test.ts`、`.spec.js`、`.spec.ts` 文件。
- 如需新增测试文件，统一创建在项目根目录下的 `test` 目录中；若 `test` 目录不存在，可在根目录创建。
- 若测试仅针对某个页面或组件，也仍然放在根目录 `test` 下，通过文件名体现归属，不要放回页面目录。
- 任务执行完毕后删除 `.test.js`、`.test.ts`、`.spec.js`、`.spec.ts` 文件。
