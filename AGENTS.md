...
- 每一个步骤开始之前都要记录开始时间，步骤结束后要结算消耗时间，最终任务完成后要列出每个步骤的消耗时间。
- 这里的“步骤”优先按可独立交付的业务单元或技术单元划分，通常包括需求理解、现状调研、方案拆分、独立页面、独立业务模块、独立功能模块、页面区块、独立组件、hooks、store、接口适配、数据结构调整、bug 修复闭环、联调、测试、回归、乱码检查、diff 复核、跨文件或跨模块迁移等内容。
- 当同一任务中存在多个业务模块时，应尽量拆分为独立步骤分别计时，便于查看每个功能模块的耗时，避免长时间卡在某一个模块上而无法定位耗时瓶颈。
- 仅包含短暂检索、文件切换、零散查看、单字段微调等准备性动作时，可合并到最近的实质步骤中，不必单独拆分计时。
- 最终汇总步骤耗时时，优先使用表格展示“步骤、开始时间、结束时间、耗时”，避免用散列清单影响阅读体验。
- 步骤耗时应精确到秒进行记录与汇总，不要使用“约”这类模糊表述。
- 最终的 `diff` 复核、中文字符串检查、乱码检测、乱码修复应视为独立收尾步骤单独计时，不得并入前一项开发步骤。
